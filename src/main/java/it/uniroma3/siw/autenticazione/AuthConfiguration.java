...
public class AuthConfiguration {
   ...
   public PasswordEncoder passwordEncoder() {
    ...
    protected SecurityFilterChain configure(final HttpSecurity http) throws Exception{
                ...
                .requestMatchers(HttpMethod.GET,"/","/login","/register","/index","/homepage","/formCercaMacchine", "/risMacchine/{id}" , "/error", "404", "/css/**", "/immagini/**").permitAll()
                // chiunque (autenticato o no) può mandare richieste POST al punto di accesso per login e register
                .requestMatchers(HttpMethod.POST, "/login","/register", "/success", "/cercaMacchine", "/error").permitAll()
